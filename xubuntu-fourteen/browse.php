<?php
	...
	if( is_category( ) ) {
		...
		if( count( $terms ) > 1 ) {
			...
			$more = __( 'See all articles from this category &raquo;', 'xubuntu' );
		}
	} elseif( is_tag( ) ) {
		...
		if( count( $terms ) > 1 ) {
			...
			$more = __( 'See all articles for this tag &raquo;', 'xubuntu' );
		}
	} else {
...
<div id="browse" class="group">
	...
	<div class="right-column">
		...
		<?php
			if( !is_home( ) ) { ?>
				<hr />
				<p class="more-link"><a href="<?php print get_permalink( get_option( 'page_for_posts' ) ); ?>"><?php _e( 'Go to the blog starting page &raquo;', 'xubuntu' ); ?></a></p>
			<? }
		?>
	</div>
</div>
